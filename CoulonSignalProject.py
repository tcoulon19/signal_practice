...
def generate_wave(sampling_frequency, time_duration, wave_frequency, snr_dB):
    """
    Generate a wave with noise. It is possible to sample the signal from an exponential distribution. In this case the noise is added to the wave and the time range is returned.

    @param sampling_frequency - Sampling frequency in Hz.
    @param time_duration - Time interval in seconds. Must be greater than 0.
    @param wave_frequency - Frequency of the wave in Hz.
    @param snr_dB - Signal to noise ratio in dB.

    @return Tuple of two NumPy arrays. First element is time range second element is wave sampled from the signal
    """
...
def add_noise(signal, snr_dB):
    ...
    alpha = np.sqrt(
        sum_signal_squared / (snr * sum_noise_squared)
    )  # alpha amplifies noise to meet SNR requirement
...
def calculate_PSD(sampled_wave, sampling_frequency, time_duration):
    ...
    PSD_values = (
        abs(dft) ** 2
    ) / fft_size  # Square dft magnitude to get PSD and normalize by sample length

    frequency_resolution = sampling_frequency / fft_size
    frequencies_to_plot = np.arange(
        -(fft_size * frequency_resolution) // 2,
        (fft_size * frequency_resolution) // 2,
...
(time_range_1, sampled_wave_1) = generate_wave(
    sampling_frequency, time_duration, wave_frequency, snr_dB_1
)
(PSD_values_1, frequencies_to_plot_1) = calculate_PSD(
    sampled_wave_1, sampling_frequency, time_duration
)
...
(time_range_2, sampled_wave_2) = generate_wave(
    sampling_frequency, time_duration, wave_frequency, snr_dB_2
)
(PSD_values_2, frequencies_to_plot_2) = calculate_PSD(
    sampled_wave_2, sampling_frequency, time_duration
)
